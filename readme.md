...
명령을 직접 입력하고 싶지 않다면 `scripts/push_to_waterfirst_repo.sh` 스크립트를 실행해 현재 브랜치를 자동으로 `https://github.com/waterfirst/korean_movie.git` 원격 저장소에 업로드할 수 있습니다.
...
> 스크립트는 인증 정보를 대신 만들어 주지 않으므로 GitHub Personal Access Token 혹은 SSH 키 등 필요한 자격 증명을 미리 준비해야 합니다.
