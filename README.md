...
1. 이 저장소를 GitHub에 푸시합니다(아래 "GitHub 저장소로 업로드하기" 참고).
2. 저장소의 **Settings ▸ Pages**로 이동합니다.
3. **Deploy from a branch**를 선택하고, **Branch**는 `main`, **Folder**는 `/docs`로 지정한 뒤 **Save**를 누릅니다.
4. Pages가 정적 파일을 처리할 때 Jekyll이 간섭하지 않도록 `docs/.nojekyll` 파일이 커밋되어 있는지 확인합니다.
5. 몇 분 뒤 생성되는 GitHub Pages URL에 접속하면 `docs` 폴더의 정적 사이트가 그대로 노출됩니다.

> GitHub Pages에 배포된 후에도 `scripts/generate_movie_dataset.py`를 실행하면 최신 데이터가 `docs/data/movies.json`에 저장되어 곧바로 배포본에 반영됩니다.

## GitHub 저장소로 업로드하기

원하는 GitHub 저장소(예: `https://github.com/waterfirst/korean_movie`)로 소스 파일을 업로드하려면 아래 단계를 따릅니다.

```bash
git remote add origin https://github.com/waterfirst/korean_movie.git  # 최초 1회만 실행
git branch -M main
git push -u origin main
```

이미 다른 원격이 설정되어 있다면 `git remote set-url origin https://github.com/waterfirst/korean_movie.git` 명령으로 교체한 뒤 `git push`를 실행하면 됩니다. HTTPS가 아닌 SSH URL을 사용하고 싶다면 `git@github.com:waterfirst/korean_movie.git`을 지정하세요.

### push 스크립트 활용하기

명령을 일일이 입력하는 대신 `scripts/push_to_waterfirst_repo.sh` 스크립트를 실행하면 현재 체크아웃된 브랜치를 자동으로 `https://github.com/waterfirst/korean_movie.git` 원격 저장소로 푸시할 수 있습니다.

```bash
./scripts/push_to_waterfirst_repo.sh               # 기본값: origin 원격, 현재 브랜치 푸시
./scripts/push_to_waterfirst_repo.sh git@github.com:waterfirst/korean_movie.git  # SSH URL 사용
REMOTE_NAME=upstream ./scripts/push_to_waterfirst_repo.sh  # 원격 이름을 origin 대신 upstream으로 지정
```

> 스크립트는 인증 정보를 자동으로 구성하지 않으므로 GitHub Personal Access Token 또는 SSH 키 등 필요한 자격 증명을 미리 준비해야 합니다.
